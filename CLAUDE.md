...
- **SSH connect froze the whole machine (~90s) on unreachable host**: `init_connection` (SSH) did a synchronous blocking `OTConnect` with `OTUseSyncIdleEvents(false)` and NO notifier. On cooperative System 7, OT then spins without ever yielding, so the Finder and every app freeze for the OS TCP timeout. Telnet already had the correct pattern; SSH never got it. Fix: port the telnet pattern — install `ssh_ot_notifier`, set `OTUseSyncIdleEvents(true)` around `OTConnect`, arm a 30s deadline, `OTCancelSynchronousCalls` on timeout/EXIT, handle `kOTCanceledErr` (timed out) and `kOTLookErr` (connection refused), then flip back to `false` + `OTSetNonBlocking` after connect so the read loop still gets `kOTNoDataErr`.
- **`host` command froze the whole machine (7+ min) on an unresolvable name**: `cmd_host` did a blocking `OTInetStringToAddress` in the main thread. Unlike endpoint calls, internet-services DNR queries NEVER deliver `kOTSyncIdleEvent`, so the notifier-based timeout never fires — and `OTCancelSynchronousCalls` (even from a Time Manager task at interrupt time) does NOT cancel the DNR query either. The blocking call therefore spins without yielding for the full OS DNS timeout (~7 min for `.local`), freezing the machine. Fix: run the DNS query in a worker thread (`WORKER_HOST`) using ASYNC mode — `OTSetAsynchronous` + `OTInstallNotifier(host_dnr_notifier)`. `OTInetStringToAddress`/`OTInetAddressToName` return `noErr` (NOT `kOTNoDataErr`) and complete via `T_DNRSTRINGTOADDRCOMPLETE`/`T_DNRADDRTONAMECOMPLETE`. The worker pumps with `YieldToAnyThread()` (main thread keeps running, no freeze) and enforces a 10s deadline (`OT_TIMEOUT_TICKS`); the notifier sets `host_done`/`host_err`. Closing the provider cancels any outstanding query. **Gotcha: pump on `err == kOTNoDataErr || err == noErr`** — async DNR returns `noErr` (0) when queued, not `kOTNoDataErr`; pumping only on `kOTNoDataErr` makes every lookup "time out" immediately.
...
- **Blocking OTConnect must install a yielding notifier or it freezes the machine**: A synchronous blocking `OTConnect` without `OTInstallNotifier` + `OTUseSyncIdleEvents(true)` spins inside Open Transport without ever yielding on cooperative System 7 — the whole machine (Finder + all apps) freezes for the OS TCP timeout (~90s) on an unreachable host. The notifier must call `YieldToAnyThread()` on `kOTSyncIdleEvent` (the only legal yield point in a blocking OT call) and `OTCancelSynchronousCalls` on a deadline. This is the SSH `ssh_ot_notifier` pattern (net.c), ported from telnet's `tcp_ot_notifier`. The same endpoint is flipped to `OTUseSyncIdleEvents(false)` + `OTSetNonBlocking` AFTER connect succeeds, so the read loop's `OTRcv` still returns `kOTNoDataErr`
